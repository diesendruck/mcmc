...
do.it <- function() {
   ...
   idx <- it
   #idx <- it[-(1:100)] # drop first 1000 as initial transient
...
#   # Prior is P(1/sig2)*P(beta|K)*P(K). In log form, there are added.
#   # Prior on 1/sig2 cancels out.
#   lprratio <- (lpbeta(b1) + (K1*log(lambda) - log(factorial(K1)))) -
#               (lpbeta(b)  + (K*log(lambda) - log(factorial(K))))
# 
#   llratio <- loglik(K1, b1, sig2) - loglik(K, b, sig2) # TODO: problem, too big

#   # Transition probabilities ratio
#   A0k1 <- A0[idx.k1,idx.k1] # Precision matrix of beta prior.
#   Vk1 <- solve(t(Xk1)%*%Xk1/sig2+A0k1)
#   mk1 <- Vk1%*%(t(Xk1)%*%y)
#   A0k <- A0[idx.k,idx.k] # Precision matrix of beta prior.
#   Vk <- solve(t(Xk)%*%Xk/sig2+A0k)
#   mk <- Vk%*%(t(Xk)%*%y)
#
#   # Recreate helper variables for beta transition probabilities
#   idx.k <- 1:(2*K+1)   # select columns (elements) for K harmonics
#   Xk <- X[,idx.k]      # Subset of design matrix, for K.
#   
#   idx.k1 <- 1:(2*K1+1)   # select columns (elements) for K+1 harmonics
#   Xk1 <- X[,idx.k1]      # Subset of design matrix, for K+1.
#
#   # Likelihood ratio
#   # P(y|K1,...) / P(y|K,...)
#   llratio <- (  ((-1/2)*(1/sig2)*sum((y-Xk1%*%b1)^2))
#               - ((-1/2)*(1/sig2)*sum((y-Xk%*%b)^2))  )
# 
#   # Prior ratio
#   # P(sig2)*P(b1|K1)*P(K1) / P(sig2)*P(b|K)*P(K)
#   lprratio <- (  ((-1/2)*(1/10)*sum(b1^2) + K1*log(lambda) - log(factorial(K1)))  
#                - ((-1/2)*(1/10)*sum(b^2) + K*log(lambda) - log(factorial(K)))  )
#
#   # P(b|K, sig2, y) * P(sig2|K, b, y) / P(b1|K1, sig2, y) * P(sig2|K1, b1, y)
#   lqratio <- (
#                # Transition probabilities under K
#                (
#                   (-(2*K+1)/2*log(2*pi) + (-1/2)*log(det(Vk)) +   # P(b|K...)
#                     (-1/2)*(t(b-mk)%*%solve(Vk)%*%(b-mk))
#                   ) + 
#                   (-(n/2+1)*((t(y-Xk%*%b)%*%(y-Xk%*%b)/2)+1) -  # P(sig2|K...)
#                     (-(1/sig2)*((t(y-Xk%*%b)%*%(y-Xk%*%b)/2)+1))
#                   )
#                )
#                # Transition probabilities under K+1
#               -( 
#                   (-(2*K1+1)/2*log(2*pi) + (-1/2)*log(det(Vk1)) +   # P(b1|K1...)
#                     (-1/2)*(t(b1-mk1)%*%solve(Vk1)%*%(b1-mk1))
#                   ) + 
#                   (-(n/2+1)*((t(y-Xk1%*%b1)%*%(y-Xk1%*%b1)/2)+1) -  # P(sig2|K1...)
#                     (-(1/sig2)*((t(y-Xk1%*%b1)%*%(y-Xk1%*%b1)/2)+1))
#                   )
#                 
#                )
#              )
...
#   lqratio <- log(0.5) - log(0.5) # Does the denominator need a 1/K, 
#                                  # because death can be any one of K deaths?
